<template name="yq-avatar">
	<view>
		<!-- 这里去掉了 @click="fSelect" 为了让外层图片做预览 -->
		<image :src="imgSrc.imgSrc" :style="[ iS ]" class="my-avatar"></image>
		<!-- 这里加上@click.stop="stopPropagation" 是为了防止点击事件冒泡到外层预览图片 -->
		<canvas @click.stop="stopPropagation" canvas-id="avatar-canvas" id="avatar-canvas" class="my-canvas" :style="{top: sT, height: csH}"
		 disable-scroll="false"></canvas>
		<canvas @click.stop="stopPropagation" canvas-id="oper-canvas" id="oper-canvas" class="oper-canvas" :style="{top: sT, height: csH}"
		 disable-scroll="false" @touchstart="fStart" @touchmove="fMove" @touchend="fEnd"></canvas>
		<canvas @click.stop="stopPropagation" canvas-id="prv-canvas" id="prv-canvas" class="prv-canvas" disable-scroll="false" 
		...
		<view class="oper-wrapper" :style="{display: sD, top:tp}">
			<view class="oper">
				<view class="btn-wrapper" v-if="sO">
					<view @click.stop="fClose" hover-class="hover" :style="{width: bW}"><text>取消</text></view>
